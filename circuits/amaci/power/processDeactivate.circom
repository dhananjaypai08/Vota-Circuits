...
template ProcessDeactivateMessages(
    ...
    signal input c1[batchSize][2];
    signal input c2[batchSize][2];
    // signal input xIncrement[batchSize];
    ...
    for (var i = 0; i < batchSize; i ++) {
        ...
        processors[i].c1[0] <== c1[i][0];
        processors[i].c1[1] <== c1[i][1];
        processors[i].c2[0] <== c2[i][0];
        processors[i].c2[1] <== c2[i][1];
        // processors[i].xIncrement <== xIncrement[i];
...
template ProcessOne(stateTreeDepth) {
    ...
    decryptCurrentIsActive.c2[1] <== stateLeaf[STATE_LEAF_C2_1_IDX];
    // decryptCurrentIsActive.xIncrement <== stateLeaf[STATE_LEAF_X_INCREMENT_IDX];
    decryptCurrentIsActive.privKey <== coordPrivKey;
    ...
    stateIndexMux.s <== validStateLeafIndex.out;
    ...
    decryptIsActive.c1[0] <== c1[0];
    decryptIsActive.c1[1] <== c1[1];
    decryptIsActive.c2[0] <== c2[0];
    decryptIsActive.c2[1] <== c2[1];
    // decryptIsActive.xIncrement <== xIncrement;
    decryptIsActive.privKey <== coordPrivKey;
    // component isActive = IsZero();
    // isActive.in <== 1 - decryptIsActive.isOdd;

    valid.out === 1 - decryptIsActive.isOdd;
